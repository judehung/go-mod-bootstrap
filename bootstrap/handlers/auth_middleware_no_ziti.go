	...
	return func(inner echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if secretProvider.IsZeroTrustEnabled() {
				// this implementation will be pick up in the build when build tag no_ziti is specified, which OpenZiti
				// packages are not included and the Zero Trust feature is not available.
				lc.Info("zero trust was enabled, but service is built without OpenZiti packages. falling back to token-based auth")
			}
