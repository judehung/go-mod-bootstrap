...
func ListenOnMode(bootstrapConfig config.BootstrapConfiguration, serverKey, addr string, t startup.Timer, server *http.Server, dic *di.Container) error {
	...
	if ok && strings.EqualFold(listenMode, ZeroTrustMode) {
		lc.Debug("detected service configured with zero trust security mode")
		secretProvider := container.SecretProviderExtFrom(dic.Get)
		if secretProvider == nil {
			return errors.New("secret provider is nil. cannot proceed with zero trust configuration")
		}
		secretProvider.EnableZeroTrust() //mark the secret provider as zero trust enabled

		jwt, jwtErr := secretProvider.GetSelfJWT()
		if jwtErr != nil {
			return fmt.Errorf("could not load jwt from secret provider while establishing zero trust overlay network: %w", jwtErr)
		}
		ozUrl := bootstrapConfig.Service.SecurityOptions[OpenZitiControllerKey]
